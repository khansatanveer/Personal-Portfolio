...
>
          ...
          {/* Desktop Navigation */}
           <div className="hidden md:flex items-center gap-1 ml-[530px]">
            {navLinks.map((link) => (
             <Link
  key={link.name}
  href={link.href}
  onClick={() => handleNavClick(link.href)}
  className="relative px-4 py-2 text-sm  lg:text-base font-medium text-gray-300 hover:text-white transition-colors duration-200 group"
>
  {link.name}
  ...
  className="block w-full px-4 py-3 text-gray-300 hover:text-white hover:bg-white/5 rounded-lg transition-all duration-200"
>
  {link.name}
</Link>
              ))}
